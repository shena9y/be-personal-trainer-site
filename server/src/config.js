...
export function createConfig(env = process.env) {
  const databasePath =
    env.DATABASE_PATH && env.DATABASE_PATH.trim() !== ""
      ? env.DATABASE_PATH === ":memory:" || path.isAbsolute(env.DATABASE_PATH)
        ? env.DATABASE_PATH
  ...
  return {
    ...
    // Railway (and most PaaS hosts) require binding to 0.0.0.0, not 127.0.0.1 —
    // 127.0.0.1 only accepts connections from inside the same container and
    // will fail their health check / routing layer.
    ...
    contactRateLimitWindowMs: toInt(
    ...
    smtpFrom:
      env.SMTP_FROM || "Be Personal Trainer <no-reply@bepersonal.example>",
